...
parser = argparse.ArgumentParser(description=
                                 'THT22 névsorok átalakítása SQl be. Csak xls kiterjesztésű fájlokkal működik.')
...
if len(sys.argv[1:]) == 0:  # Ha nincs semmi megadva akkor a helpet kiijuk
    parser.print_help()
    parser.exit()
...
try:
    for file in filenames:  # Kezdődik a lényeg.
        ...
        book = xlrd.open_workbook(file,logfile) # a lista.txt-ben egy sor ami egy fájlnévre mutat, azt megnyitjuk mint xls.
        sheet = book.sheet_by_index(0)  # munkalap beállítása az elsőre
        maxsor: int = sheet.nrows   # a max sorok megállapítása
        maxcol: int = sheet.ncols   # a max oszlopok megállapítása.
        ...
        if sheet.cell_type(1, 2) == 1:  # megnézzük szöveg van e a társulat nevében 1 sor második oszlop.
            ...
            if sqlceg is None: # ha nincs akkor beírjuk mint új cég.
                ...
                sqlfile.write('\n')
                id = conn.beszur(sql)
                db_back_file.write('DELETE FROM ceglista WHERE sorsz = {};\n'.format(id)) # a backuphoz írjuk a ceg sorszámot.
            else:
                id = sqlceg['sorsz']
                # Cégnév beszúrása, ha még nins a listában.
